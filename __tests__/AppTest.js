...
describe('로또 유효성 검증 테스트', () => {
  ...
  describe('구입 금액 검증 테스트', () => {
    test.each([
      ...
      ['0 이하인 경우', '0', '[ERROR] 0 이하의 금액을 입력받았습니다.'],
      ['빈 값 입력 시', '', '[ERROR] 빈 값을 입력하였습니다.'],
    ])('%s 예외 발생', (_, input, expectedMessage) => {
      expect(() => Validator.validatePurchaseAmount(input)).toThrow(
        expectedMessage
      );
    });
  ...
  describe('당첨 번호 검증 테스트', () => {
    ...
    test('정상 입력 시 배열 반환', () => {
      expect(Validator.validateWinningNumbers('1,2,3,4,5,6')).toEqual([
        1, 2, 3, 4, 5, 6,
      ]);
    });
  });
  ...
  describe('보너스 번호 검증 테스트', () => {
    ...
    test.each([
      ...
      ['당첨 번호와 중복될 경우', '3', '[ERROR] 당첨 번호와 중복되었습니다.'],
    ])('%s 예외 발생', (_, input, expectedMessage) => {
      expect(() =>
