...
- [x] 클래스를 통해 코드를 구조적으로 분리한다.
- [x] 입력 / 검증 / 출력의 책임을 명확히 분리한다.
- [x] 메서드가 한 가지 기능을 하도록 한다.
- [x] App에서 입력과 출력을 분리하고, 핵심 로직을 독립시켜본다.
- [x] 모듈화를 시도해본다.
- [x] 상수를 정의하고 의미있는 이름을 부여하여 해당 값이 어떤 역할을 하는지 명확히 드러낸다.
- [x] 예외 메시지를 한 곳에서 관리해본다.

---

## 기능 구현 목록

입력–검증–출력의 역할을 명확히 분리하고, 클래스를 통해 구조적으로 코드를 설계하였다.
